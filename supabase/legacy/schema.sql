...
create table if not exists profiles (
  ...
  membership_tier text not null default 'free' check (membership_tier in ('free','circle','inner-circle','founder')),
...
-- If this table already existed from an earlier version of this schema, widen the tier check:
...
alter table profiles add constraint profiles_membership_tier_check
  check (membership_tier in ('free','circle','inner-circle','founder'));
...
-- ============================================================
-- COMMUNITY TAB — Instagram-style posts, separate from the private journal
-- ============================================================
...
-- ============================================================
-- WORKOUTS HUB — workouts, meal plans, grocery lists. Gated by membership.
-- ============================================================
...
create table if not exists workouts (
  ...
  pillar text not null default 'Body' check (pillar in ('Body','Identity','Mindset','Faith')),
  video_url text,
  instructions text,
  image_url text,
  pdf_url text,
...
-- community_posts: everyone signed in can read; own inserts; only founders/admins can pin
drop policy if exists "view community posts" on community_posts;
create policy "view community posts" on community_posts for select using (auth.role() = 'authenticated');
drop policy if exists "insert own community post" on community_posts;
create policy "insert own community post" on community_posts for insert with check (auth.uid() = user_id);
drop policy if exists "update own or founder pin" on community_posts;
create policy "update own or founder pin"
  on community_posts for update
  using (
    auth.uid() = user_id
    or exists (select 1 from profiles where id = auth.uid() and (membership_tier = 'founder' or is_admin))
  );
drop policy if exists "delete own community post" on community_posts;
create policy "delete own community post" on community_posts for delete using (auth.uid() = user_id);

drop policy if exists "view community reactions" on community_reactions;
create policy "view community reactions" on community_reactions for select using (auth.role() = 'authenticated');
drop policy if exists "insert own community reaction" on community_reactions;
create policy "insert own community reaction" on community_reactions for insert with check (auth.uid() = user_id);
drop policy if exists "delete own community reaction" on community_reactions;
create policy "delete own community reaction" on community_reactions for delete using (auth.uid() = user_id);

drop policy if exists "view community comments" on community_comments;
create policy "view community comments" on community_comments for select using (auth.role() = 'authenticated');
drop policy if exists "insert own community comment" on community_comments;
create policy "insert own community comment" on community_comments for insert with check (auth.uid() = user_id);
drop policy if exists "update own or founder pin comment" on community_comments;
create policy "update own or founder pin comment"
...
-- workouts / meal plans / grocery lists: readable by all signed-in users (paywalled in the UI, not the DB,
-- since the app needs to show a locked preview); only admins manage
drop policy if exists "view workouts" on workouts;
create policy "view workouts" on workouts for select using (auth.role() = 'authenticated');
drop policy if exists "admins manage workouts" on workouts;
create policy "admins manage workouts"
  on workouts for all
  using (exists (select 1 from profiles where id = auth.uid() and is_admin))
  with check (exists (select 1 from profiles where id = auth.uid() and is_admin));

drop policy if exists "view meal plans" on meal_plans;
create policy "view meal plans" on meal_plans for select using (auth.role() = 'authenticated');
...
drop policy if exists "view grocery lists" on grocery_lists;
create policy "view grocery lists" on grocery_lists for select using (auth.role() = 'authenticated');
drop policy if exists "admins manage grocery lists" on grocery_lists;
create policy "admins manage grocery lists"
...
-- ============================================================
-- MAKE YOURSELF THE FOUNDER
-- Run this ONE line after you've signed up, replacing the email.
-- Founder = highest tier, full access, pin posts/comments everywhere.
-- ============================================================
-- update profiles set membership_tier = 'founder', is_admin = true where email = 'you@example.com';
